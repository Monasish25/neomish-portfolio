...
// Receives video file + metadata, uploads to Mux, inserts project row
//
...
serve(async (req) => {
  ...
  try {
    ...
    const dur = formData.get("dur")?.toString() ?? "";
    const year = formData.get("year")?.toString() ?? "";
    const blurb = formData.get("blurb")?.toString() ?? "";
    const aspect_ratio = formData.get("aspect_ratio")?.toString() ?? "16:9";
    ...
    // Step 1: Create a Mux asset via direct upload
    const createRes = await fetch("https://api.mux.com/video/v1/assets", {
      ...
      body: JSON.stringify({
        input: [{ url: "pending" }],  // We'll use upload API instead
        playback_policy: ["public"],
        encoding_tier: "baseline",
      }),
    });

    // Actually, use Direct Uploads for large files
    ...
    // Step 2: PUT the video file to the Mux upload URL
    ...
    if (!putRes.ok) {
      return new Response(JSON.stringify({ error: "Failed to upload video to Mux" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // Step 3: Poll for asset readiness (the upload creates an asset)
    let assetId = null;
    let playbackId = null;
    let attempts = 0;
    ...
    const { data: project, error: insertError } = await adminSupabase
      .from("projects")
      .insert({
        title, client, cat, role, tools, dur, year, blurb, aspect_ratio,
